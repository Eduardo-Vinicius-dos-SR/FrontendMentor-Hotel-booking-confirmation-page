...
export default function NavItem({ href, icon, label, badge }: NavItemProps) {
    return (
        <li className="w-full">
            <NavLink to={href} className={({ isActive }) => `flex items-center justify-start gap-3 lg:gap-2 font-sans font-semibold rounded-md text-base lg:text-[13px] 2xl:text-sm p-3 lg:p-2 2xl:p-3 ${isActive ? "bg-[var(--Neutral-0)] text-[var(--Neutral-900)]" : "text-[var(--Neutral-600)] hover:bg-[var(--Neutral-0)]"}`}>
